...
export function NewContactForm({ events }: { events: EventOption[] }) {
        ...
        {events.length > 0 && (
          <div>
            ...
            <select
              ...
              className="w-full h-12 px-4 rounded-lg bg-[#0F2337] border border-[#1E3A5F] text-[#F1F5F9] focus:outline-none focus:border-[#3B82F6] transition-colors appearance-none"
            >
              <option value="">— no event —</option>
              {events.map((e) => {
                const date = formatShortDate(e.start_date);
                return (
            ...
            {priorityOptions.map((opt) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-semibold text-[#F1F5F9] text-sm">{opt.label}</span>
                  <span className="text-[10px] text-[#94A3B8] mt-0.5">{opt.description}</span>
                </button>
              );
            })}
          </div>
